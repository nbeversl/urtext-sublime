...
class ProjectList():
    ...
    def add_project(self, project):
        if os.path.isdir(project.entry_point):
            if project.entry_point in self.get_all_paths():
                ...
                return
        if not project._get_included_paths() and not os.path.isdir(project.entry_point):
            print('NO PATHS')
            return
    ...
    def _get_project_from_path(self, path):
        ...
        for project in self.projects:
            if path in project._get_included_paths():
                return project
    ...
    def set_current_project(self, title_or_path):
        ...
        if ( not self.current_project ) or ( 
            ...
            project_paths = self.current_project._get_included_paths()
            if project_paths:
                self.current_project.on_project_activated(project_paths[0])
    ...
    def get_current_project(self, path):
        for project in self.projects:
            if project.path in project._get_included_paths():
                return project
        return None
    ...
    def move_file(self, 
        ...
        new_filename = os.path.join(
            destination_project._get_included_paths()[0],
